...
export function startEleSurfaceSampling() {
    // renderer.setAnimationLoop(render);
    elContent = document.querySelector('.surface-sampling-content');
    scene = new THREE.Scene();
    camera = new THREE.PerspectiveCamera(75, elContent.offsetWidth / elContent.offsetHeight, 0.1, 1000);
    ...
    let elephant = null;
    let paths = [];
    new OBJLoader().load("models/Elephant_Model.obj", (obj) => {
        ...
        renderer.setAnimationLoop(render);
    }, (xhr) => console.log((xhr.loaded / xhr.total) * 100 + "% loaded"), (err) => {
        console.log('oops');
        console.error(err)
    });
    ...
    const materials = [new THREE.LineBasicMaterial({
        color: 0xFAAD80, transparent: true, opacity: 0.5
    }), new THREE.LineBasicMaterial({
        color: 0xFF6767, transparent: true, opacity: 0.5
    }), new THREE.LineBasicMaterial({
        color: 0xFF3D68, transparent: true, opacity: 0.5
...
export function stopEleSurfaceSampling() {
