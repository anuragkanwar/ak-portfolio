...
import {mousepos, winsize, dist} from "../utils/index.js";
...
export default class ButtonCtrl extends EventEmitter {
    constructor(el) {
        ...
        this.renderedStyles = {
            tx: {previous: 0, current: 0, amt: 0.1}, ty: {previous: 0, current: 0, amt: 0.1}
        };
        // button state (hover)
        this.state = {
            hover: false
        ...
        this.DOM.activator.style.width = `${this.rect.width * 2}px`;
    ...
    render() {
        ...
        const distanceMouseButton = dist(mousepos.x + window.scrollX, mousepos.y + window.scrollY, this.rect.left + this.rect.width / 2, this.rect.top + this.rect.height / 2);
        ...
        for (const key in this.renderedStyles) {
            this.renderedStyles[key].previous = interpolate(this.renderedStyles[key].previous, this.renderedStyles[key].current, this.renderedStyles[key].amt);
        }
    ...
    enter() {
        ...
        gsap
            ...
            .to(this.DOM.textinner, 0.15, {
                ease: 'Power2.easeIn', opacity: 0, x: '20%'
            })
            .to(this.DOM.textinner, 0.2, {
                ease: 'Expo.easeOut', opacity: 1, startAt: {x: '-20%'}, x: '0%'
            });
    }
